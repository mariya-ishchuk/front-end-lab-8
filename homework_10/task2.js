...
function fighter(combat) {
  ...
  var fighter = {
    getName: () => combatStats.name,
    block: () => Math.random() >= 0.5,
    getStats: () => combatStats,
    getCombatHistory: () => combatHistory,
    fight: enemy => {
      if (enemy.block() === false) {
        return false;
      } else {
        if (fighter.getStats().attack > enemy.getStats().hp) {
          fighter.getCombatHistory().wins++;
          enemy.getCombatHistory().loses++;
        } else {
          enemy.getStats().hp = enemy.getStats().hp - fighter.getStats().attack;
        }
        return true;
      }
    }
